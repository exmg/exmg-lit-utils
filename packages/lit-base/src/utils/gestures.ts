 ...
 * @return {void}
 */
function setupTeardownMouseCanceller(setup?: any) {
 ...
 * @return {void}
 */
export function register(recog: any) {
  // @ts-ignore
 ...
 */
function _findRecognizerByEvent(evName: string) {
  for (let i = 0, r; i < recognizers.length; i++) {
    r = recognizers[i];
 ...
 * @return {void}
 */
function _fire(target: EventTarget, type: string, detail?: any) {
 ...
 * @return {void}
 */
function downupFire(type: string, target: EventTarget, event: any, preventer?: any) {
  if (!target) {
    return;
  }
  _fire(target, type, {
 ...
 * @return {void}
 */
function trackForward(info: any, e: any, preventer?: any) {
